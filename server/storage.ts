...
export class MemStorage implements IStorage {
  ...
  async searchDocuments(query: string): Promise<DocumentChunk[]> {
    try {
      ...
      const results = this.documentChunks.filter(
        (chunk) =>
      ...
      return results.sort((a, b) => {
        const aMatches = (
          a.content.toLowerCase().match(new RegExp(normalizedQuery, "g")) || []
        ).length;
        const bMatches = (
          b.content.toLowerCase().match(new RegExp(normalizedQuery, "g")) || []
        ).length;
