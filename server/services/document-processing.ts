...
export async function loadInitialData(): Promise<void> {
  ...
  const cirurgiasSource =
    "Orientações e Fluxo para Cirurgias de Afirmação de Gênero";
  ...
  log(
    `Loaded ${allChunks.length} document chunks and ${faqs.length + commonQuestions.length} FAQs`,
  );
}
